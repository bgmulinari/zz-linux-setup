...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    float lineLength = length(delta);
     ...
     if (lineLength > minDist) {
        ...
        // Calculate fade based on position along the trail (0 at tail, 1 at head)
        vec2 tailToHead = head_center - tail_center;
        float trailLen = length(tailToHead);
        float fadeFactor = 1.0;
        if (trailLen > 0.001) {
            vec2 tailToPixel = vu - tail_center;
            float projectedDist = dot(tailToPixel, tailToHead / trailLen);
            fadeFactor = clamp(projectedDist / trailLen, 0.0, 1.0);
        }

        vec4 trail = TRAIL_COLOR;
        float trailAlpha = antialising(sdfTrail) * fadeFactor * (1.0 - progress);
